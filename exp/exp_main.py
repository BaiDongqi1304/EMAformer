...
from models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, PatchTST, PatchMixer, AttenConvMixer, AttenMixer, AttenMixer2, DAttenMixer,DAttenMixerAB1,DAttenMixerAB2,DAttenMixerAB3,DAttenMixerAB4
...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        return loss
class arctant_Loss(nn.Module):
    def __init__(self, pred_len=192,device='cuda'):
        super(arctant_Loss, self).__init__()
        self.ratio = np.array([-1 * math.atan(i + 1) + math.pi / 4 + 1 for i in range(pred_len)])
        self.ratio1 = torch.tensor(self.ratio).unsqueeze(-1).to(device)
        self.l1_loss = nn.L1Loss()
    def forward(self, outputs, targets):
        outputs = outputs * self.ratio1
        targets = targets * self.ratio1
...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self):
        model_dict = {
            ...
            'AttenConvMixer': AttenConvMixer,
            'AttenMixer': AttenMixer,
            'AttenMixer2': AttenMixer2,
            'DAttenMixer': DAttenMixer,
            'DAttenMixerAB1':DAttenMixerAB1,
            'DAttenMixerAB2':DAttenMixerAB2,
            'DAttenMixerAB3':DAttenMixerAB3,
            'DAttenMixerAB4':DAttenMixerAB4
        }
    ...
    def _select_criterion(self):
        if self.args.loss_flag==1: # loss_flag 0 for MSE, 1 for MAE, 2 for both of MSE & MAE, 3 for SmoothL1loss
            ...
            criterion = arctant_Loss(self.args.pred_len, self.device)
        else:
