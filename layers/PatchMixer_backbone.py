...
class Backbone(nn.Module):
    def __init__(self, configs,revin = True, affine = True, subtract_last = False):
        ...
        self.W_P = nn.Linear(self.d_model * 2, self.d_model)
    ...
    def forward(self, x):
        ...
        for PatchMixer_block in self.PatchMixer_blocks:
            x = PatchMixer_block(x)                                                   # x: [batch * n_val, a, d_model]
        x = self.head1(x)                                                           # x: [batch * n_val, forecasting]
        #x = u + x
        x = torch.reshape(x, (x.shape[0]//self.nvals, self.nvals, -1))               # x: [batch, n_val, pred_len]
