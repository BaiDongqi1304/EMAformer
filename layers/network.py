...
class Network(nn.Module):
    ...
    def forward(self, s, t):
        ...
        # Patch Embedding
        s = self.fc1(s)
        s = self.gelu1(s) # [B*C, patch_num, patch_len * patch_len]
        s = self.bn1(s)

        res = s

        # CNN Depthwise
        s = self.conv1(s) # [B*C, patch_num, patch_len]
        s = self.gelu2(s)
        s = self.bn2(s)

        # Residual Stream
        res = self.fc2(res)
        s = s + res #[B*C, patch_num, patch_len]
