...
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,batch: int, nvals: int,
        ...
        # Backbone
        self.backbone = TSTiEncoder(c_in, patch_num=patch_num, patch_len=patch_len, max_seq_len=max_seq_len,context_window=context_window,batch = batch, nvals = nvals,
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        # model
        z = self.backbone(z)  # z: [bs x nvars x seq_len]
        #z = z.permute(0, 2, 1)  # z: [bs * nvars x patch_num x d_model] d_model = 256
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,context_window=336,batch = 256, nvals = 7,
        ...
        # Input encoding
        q_len = patch_num
        self.W_P = nn.Linear(context_window, d_model)  # Eq 1: projection of feature vectors onto a d-dim vector space
        self.seq_len = q_len

        # Positional encoding
        self.W_pos = positional_encoding(pe, learn_pe, 1, d_model)
    ...
    def forward(self, x) -> Tensor:  # x: [bs x nvars x seq_len]
        ...
        x = self.W_P(x)  # x: [bs x nvars x d_model]
        #print(f"[Debug] Shape of x before reshape: {x.shape}, Elements: {x.numel()}")
        u = torch.reshape(x, (x.shape[0],x.shape[1], 1, x.shape[2]))
        v = torch.reshape(u, (u.shape[0] * u.shape[1], u.shape[2], u.shape[3]))  # u: [bs * nvars x patch_num x d_model]
        v = self.dropout(v + self.W_pos)  # u: [bs * nvars x patch_num x d_model]
        #print(f"[Debug] Shape of u before reshape: {u.shape}, Elements: {u.numel()}")

        # Encoder
        z = self.encoder(v)  # z: [bs * nvars x patch_num x d_model]
        #print(f"[Debug] Shape of z before reshape: {z.shape}, Elements: {z.numel()}")
